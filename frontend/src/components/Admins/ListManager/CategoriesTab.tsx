...
const CategoriesTab = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        showToast("error", response.message || "Không thể tải danh mục");
      }
    } catch (error: any) {
  ...
  const generateCategoryId = () => {
    const maxId = categories.reduce((max, category) => {
      const match = category.category_id.match(/^CAT(\d+)$/);
      if (match) {
    ...
    return `CAT${String(newNum).padStart(3, '0')}`;
  };

  const handleSave = async () => {
    try {
      if (editingCategory) {
        const response = await adminService.updateCategory(editingCategory.category_id, formData);
        if (response.success) {
        ...
        const autoId = generateCategoryId();
              ...
              <div className="flex items-center gap-3">
                {category.icon ? (
                  category.icon.startsWith("http") ? (
                    ...
                    <img src={`${import.meta.env.VITE_API_BASE_URL || "http://localhost:3000"}${category.icon}`} alt={category.name} className="w-12 h-12 object-contain rounded" />
                  ) : (
