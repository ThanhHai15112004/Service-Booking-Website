...
export default function HotelLandingPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch featured hotels and popular destinations
        const [hotelsRes, destinationsRes] = await Promise.all([
          getFeaturedHotels({ limit: 6, minStars: 4 }),
        ...
        if (destinationsRes.success && destinationsRes.items) {
          setPopularDestinations(destinationsRes.items);
        }
      } catch (error) {
  ...
  const scrollToSearch = () => {
    ...
    setTimeout(() => {
      const destinationInput = searchBarRef.current?.querySelector('input[placeholder*="Bạn muốn đến đâu"]');
      if (destinationInput) {
